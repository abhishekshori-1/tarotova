...
export function ReadingExperience({ id, journey, onChange }: { id: string; journey?: { suggestions: string[] }; onChange?: () => void }) {
  ...
  return (
    <article className={journey ? "min-w-0" : "mx-auto max-w-5xl px-6 pb-16 pt-8"}>
      ...
      {result.followupSupport && <section className="panel mt-6 p-6"><h2 className="text-2xl">{result.followupSupport.heading}</h2>{result.followupSupport.body.map((p) => <p key={p} className="prose-measure mt-4">{p}</p>)}<ul className="mt-5 space-y-3">{result.followupSupport.resources.map((r) => <li key={r.label}>{r.href ? <a href={r.href} className="underline">{r.label}</a> : r.label} {r.detail}</li>)}</ul></section>}
      {!result.followupSupport && interpretation && <InterpretationPanel view={interpretation} onRetry={retry} retryUsed={retryUsed} />}
      ...
      {showReading && (
        <>
          ...
          {(interpretation?.status === "succeeded" || interpretation?.status === "not_applicable") && <FollowupPanel readingId={id} focus={result.focus} suggestions={journey?.suggestions} onChange={() => { void getResult(id).then((r) => setResult(r)).catch(() => {}); onChange?.(); }} />}
        </>
      )}

      {!journey && <div className="mt-10 flex flex-col gap-3 sm:flex-row sm:items-center">
        <Link href="/" prefetch={false} className="btn-primary inline-flex items-center px-6 py-3 text-sm">
