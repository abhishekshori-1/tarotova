...
- **No accounts, no production deploy.** `tarotova.com` is registered
  (Namecheap) — that one item is done. Supabase, Resend, Turnstile, and a
  hosting account (Vercel or Cloudflare Workers, undecided as of this
  writing) still don't exist, DNS hasn't been pointed anywhere, and nothing
  has been deployed.
