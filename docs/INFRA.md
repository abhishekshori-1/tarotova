...
| `GENERATION_REPAIR_PROVIDER` | Config | Optional: repairs flagged fields by a vendor other than the writer chain (that vendor's `*_ANSWER_MODEL` applies). Unset keeps the writer chain; a misconfigured value is logged and the chain repairs. Replaces the repair call; still one repair and one fresh review per attempt. |
